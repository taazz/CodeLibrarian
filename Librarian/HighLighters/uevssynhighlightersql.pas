//Copied here from trunc in order to add support for postgres must be deleted and reverted
// back to lcl file when support for postgres is released will it be on the new 1.2 need to test it.
{-------------------------------------------------------------------------------
...
$Id: synhighlightersql.pas 41303 2013-05-19 18:03:28Z martin $
...
unit uEvsSynhighlightersql;

{.$DEFINE SYN_MBCSSUPPORT} {.$I SynEdit.inc}
...
type
  ...
  TEvsSynSQLSyn = class(TSynCustomHighlighter)
  private
    fRange: TRangeState;
    fLine: PChar;
    fLineNumber: Integer;
...
function TEvsSynSQLSyn.KeyHash(ToHash: PChar): Integer;
var
  Start: PChar;
begin
  Result := 0;
...
function TEvsSynSQLSyn.KeyComp(const aKey: string): Boolean;
var
  i: integer;
  pKey1, pKey2: PChar;
begin
...
function TEvsSynSQLSyn.IdentKind(MayBe: PChar): TtkTokenKind;
var
  Entry: TSynHashEntry;
begin
  fToIdent := MayBe;
...
procedure TEvsSynSQLSyn.MakeMethodTables;
var
  I: Char;
begin
  for I := #0 to #255 do
...
constructor TEvsSynSQLSyn.Create(AOwner: TComponent);
begin
...
destructor TEvsSynSQLSyn.Destroy;
begin
  fKeywords.Free;
  fTableNames.Free;
  inherited Destroy;
...
procedure TEvsSynSQLSyn.Assign(Source: TPersistent);
begin
  inherited Assign(Source);
  if (Source is TEvsSynSQLSyn) then
    SQLDialect := TEvsSynSQLSyn(Source).SQLDialect;
end;

procedure TEvsSynSQLSyn.SetLine(const NewValue: string; LineNumber: Integer);
begin
  inherited;
...
procedure TEvsSynSQLSyn.AndSymbolProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
...
procedure TEvsSynSQLSyn.AsciiCharProc;
begin
...
procedure TEvsSynSQLSyn.CRProc;
begin
  fTokenID := tkSpace;
  Inc(Run);
...
procedure TEvsSynSQLSyn.EqualProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  if fLine[Run] in ['=', '>'] then Inc(Run);
...
procedure TEvsSynSQLSyn.GreaterProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  if fLine[Run] in ['=', '>'] then Inc(Run);
...
procedure TEvsSynSQLSyn.IdentProc;
begin
...
procedure TEvsSynSQLSyn.LFProc;
begin
  fTokenID := tkSpace;
  inc(Run);
end;

procedure TEvsSynSQLSyn.LowerProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  case fLine[Run] of
...
procedure TEvsSynSQLSyn.MinusProc;
begin
  Inc(Run);
...
procedure TEvsSynSQLSyn.NullProc;
begin
  fTokenID := tkNull;
end;

procedure TEvsSynSQLSyn.NumberProc;
begin
  inc(Run);
  fTokenID := tkNumber;
...
procedure TEvsSynSQLSyn.OrSymbolProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  if fLine[Run] in ['=', '|'] then Inc(Run);
...
procedure TEvsSynSQLSyn.PlusProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  if fLine[Run] in ['=', '+'] then Inc(Run);
...
procedure TEvsSynSQLSyn.SlashProc;
begin
  Inc(Run);
  case fLine[Run] of
    '*':
...
procedure TEvsSynSQLSyn.SpaceProc;
begin
  fTokenID := tkSpace;
  repeat
    Inc(Run);
...
procedure TEvsSynSQLSyn.StringProc;
begin
  fTokenID := tkString;
  Inc(Run);
...
procedure TEvsSynSQLSyn.SymbolProc;
begin
  Inc(Run);
  fTokenID := tkSymbol;
end;

procedure TEvsSynSQLSyn.SymbolAssignProc;
begin
  fTokenID := tkSymbol;
  Inc(Run);
  if fLine[Run] = '=' then Inc(Run);
...
procedure TEvsSynSQLSyn.VariableProc;
var
  i: integer;
begin
...
procedure TEvsSynSQLSyn.UnknownProc;
begin
...
procedure TEvsSynSQLSyn.AnsiCProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
function TEvsSynSQLSyn.IsKeyword(const AKeyword: string): boolean;
var
  tk: TtkTokenKind;
begin
...
function TEvsSynSQLSyn.GetDefaultAttribute(Index: integer):
...
function TEvsSynSQLSyn.GetEol: Boolean;
begin
  Result := fTokenID = tkNull;
end;

function TEvsSynSQLSyn.GetRange: Pointer;
begin
  Result := Pointer(PtrInt(fRange));
end;

function TEvsSynSQLSyn.GetToken: string;
var
  Len: LongInt;
begin
  Result := '';
...
procedure TEvsSynSQLSyn.GetTokenEx(out TokenStart: PChar; out TokenLength: integer);
begin
  TokenLength:=Run-fTokenPos;
  TokenStart:=FLine + fTokenPos;
end;

function TEvsSynSQLSyn.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
end;

function TEvsSynSQLSyn.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case GetTokenID of
...
function TEvsSynSQLSyn.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;

function TEvsSynSQLSyn.GetTokenPos: Integer;
begin
  Result := fTokenPos;
end;

procedure TEvsSynSQLSyn.ResetRange;
begin
  fRange := rsUnknown;
end;

procedure TEvsSynSQLSyn.SetRange(Value: Pointer);
begin
  fRange := TRangeState(PtrUInt(Value));
end;

function TEvsSynSQLSyn.GetIdentChars: TSynIdentChars;
begin
...
class function TEvsSynSQLSyn.GetLanguageName: string;
begin
  Result := SYNS_LangSQL;
end;

procedure TEvsSynSQLSyn.DoAddKeyword(AKeyword: string; AKind: integer);
var
  HashValue: integer;
begin
...
procedure TEvsSynSQLSyn.SetTableNames(const Value: TStrings);
begin
  fTableNames.Assign(Value);
end;

procedure TEvsSynSQLSyn.TableNamesChanged(Sender: TObject);
begin
  InitializeKeywordLists;
end;

procedure TEvsSynSQLSyn.PutTableNamesInKeywordList;
var
  i: Integer;
  Entry: TSynHashEntry;
begin
...
procedure TEvsSynSQLSyn.InitializeKeywordLists;
begin
  fKeywords.Clear;
...
procedure TEvsSynSQLSyn.SetDialect(Value: TSQLDialect);
begin
...
function TEvsSynSQLSyn.GetSampleSource: String;
begin
  Result:= '';
  case fDialect of
    sqlPostgres:
...
initialization
  MakeIdentTable;
  RegisterPlaceableHighlighter(TEvsSynSQLSyn);
