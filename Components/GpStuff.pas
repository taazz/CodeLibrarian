...
uses
  Variants;
{$ELSE}{$IFDEF FPC} Uses variants;{$ENDIF}
...
begin
  if gtLogRef then
    {$IFDEF FPC}
    Debugln(Format('TGpTraceable.Destroy: [%s]', [ClassName]));
    {$ELSE}
...
begin
  ...
  if gtLogRef then
    {$IFDEF FPC}
    DebugLn(Format('TGpTraceable._AddRef: [%s] %d', [ClassName, Result]));
    {$ELSE}
...
begin
  ...
  if gtLogRef then
    {$IFDEF FPC}
    DebugLn(Format('TGpTraceable._Release: [%s] %d', [ClassName, Result]));
    {$ELSE}
